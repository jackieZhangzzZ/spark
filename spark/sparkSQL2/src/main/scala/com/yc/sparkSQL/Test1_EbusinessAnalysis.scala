...
object Test1_EbusinessAnalysis {
  def main(args: Array[String]): Unit = {
    ...
    //数据处理
    val userLinesDataset = spark.read.textFile(filepath+"Users_Data.log")

    val userDataset = userLinesDataset.map(line=>{
      val fields = line.split(",")
      val userID = fields(0)
      val userName = fields(1)
      val registeredTime = fields(2)
      User(userID,userName,registeredTime)
    })
    val userDataFrame = userDataset.toDF()
    userDataFrame.createTempView("user")
    val result = spark.sql("select userID,userName,registeredTime from user order by userID desc")
    result.show()
    spark.stop()
  }
}
case class User(userID:String,userName:String,registeredTime:String)